...
int queen::check(int (intWhiteBoard[8][8]), int (intBlackBoard[8][8]), int move, int scale)
{
...
int queen::determine_move(sf::Vector2f next_position, int& scale)
{
	...
	if (color == 'w')
	{
		...
		else if (std::abs(difference_x) == std::abs(difference_y) && difference_x < 0 && difference_y < 0)
		{
			scale = -difference_x;
			return 5; // Up-Left
		}
